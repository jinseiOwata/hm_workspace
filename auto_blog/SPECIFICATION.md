...
最終更新: 2026-09-25 夕方(PR #14 マージ後。デザイン刷新・広告掲載・YouTube 告知の本番稼働後の状態)
...
**アフィリエイト枠(1記事2か所)**
- 対象: `config.json` の `affiliates[]` のうち `html` か `url` が設定済みで、`keywords` のどれかが記事の **タイトル・説明文・タグ・元テーマ** に含まれるもの(設定順 = 優先順)。
  本文は見ない(2026-09-26、PDF要約の記事の本文に「会議」が出てきて議事録向けの広告が出たため変更)
...
**デザイン**(`static/style.css` とテンプレート)
- ダーク基調に紫→シアン→ミントのグラデーションを使った「AIらしい」見た目。背景にうっすらしたグリッドと光のにじみ(動きは `prefers-reduced-motion` で停止)
- フォント: Google Fonts の Noto Sans JP / Inter / JetBrains Mono
- ヘッダー: 固定表示・半透明。グラデーションのロゴマーク(インラインSVG)。画面上端に読み進み具合のバー
- トップ: 大見出し・説明・特徴チップ(毎朝更新 / コピペで使えるプロンプト / 公開記事数)のヒーローと、記事カードのグリッド
- 記事カード: 公開2日以内は「NEW」、日付、読了時間(本文の文字数 ÷ 500字/分)、タグ最大3つ
- 記事ページ: 見出しエリア、目次(`##` 見出しが3つ以上ある記事のみ、最初の見出しの直前)、見出しの装飾、
  コードブロックは本文との境界がひと目でわかるよう、明るめの地色・シアンの太枠・グラデーションの「PROMPT」見出し帯で囲み、
  見出し帯の右端に白い「コピー」ボタンを付ける(小さなインライン JS。押すと「コピーしました」と表示)
- スマホ幅(〜600px)ではカードを1列にし、キャッチコピーを隠す
...
| トリガー | 動作 |
|---|---|
| `schedule`(毎日 06:17 JST、予備 07:47 / 09:17 / 11:47 JST) | 記事生成 → commit → ビルド → デプロイ。予備の実行は、本日分の記事があれば生成をスキップ(ビルド・デプロイ・告知確認のみ) |
| `workflow_dispatch`(手動実行) | 同上。入力 `force` を ON にすると本日分があっても追加生成 |
| `push`(main の `auto_blog/**` か workflow 自体が変わったとき) | 記事生成・告知はせず、ビルド → デプロイのみ |

- GitHub の定期実行は遅延・取りこぼしが多く、2026-09-26 朝は 06:17 の実行が 08:16 時点でも始まらなかった。そのため同じ朝に予備の実行を3回入れている
...
GitHub Pages 自体にはアクセス数を見る機能がないため、外部の計測サービスを使う(どちらも無料。未設定なら計測タグは出力しない)。
...
- Google アナリティクスを設定すると、プライバシーポリシーの「アクセス解析について」に Google アナリティクスの利用と Google の説明ページへのリンクが自動で入る
- 広告の表示・クリック・成果は A8.net の管理画面の「レポート」で確認する

## 掲載中の広告(A8.net)

`config.json` の `affiliates[]`(上から優先、全15件)。どれもテキスト広告で、Notta のみバナー(300×250)。コードは A8.net 発行のまま改変しない。

| 優先 | 広告(報酬) | 出す記事(keywords) | 枠 |
|---|---|---|---|
| 1 | PLAUD NOTE「世界初ChatGPT-4連携AIボイスレコーダー PLAUD NOTE」(購入10〜13%) | 議事録・会議・文字起こし・録音・ボイスレコーダー | 途中 |
| 2 | ZENCHORD1「【ZENCHORD1】」(購入7%) | 議事録・会議・文字起こし | 途中 |
| 3 | PLAUD NOTE「6ヶ月で全世界5万ユーザー＆12億円売り上げAIボイスレコーダー PLAUD NOTE」 | 1と同じ | 末尾 |
| 4 | Notta ZENCHORD1 バナー 300×250 | 議事録・会議・文字起こし | 末尾 |
| 5 | 英語添削アイディー(新規購入・定額プラン20%) | 英語・英文・英作文・英会話・TOEIC | 途中・末尾 |
| 6 | Aiarty Image Enhancer(初回購入30%、年間/永久ライセンスのみ) | 画像生成・アイキャッチ・高画質・画像編集・写真 | 途中・末尾 |
| 7 | ExpressVPN(新規購入4,500円) | VPN・公共Wi-Fi・フリーWi-Fi・海外旅行・海外出張 | 末尾 |
| 8 | GMKtec(ミニPC、購入5%) | ミニPC・パソコン選び・PC選び・ローカルLLM | 末尾 |
| 9 | EarFun(イヤホン、購入8%) | イヤホン・ノイズキャンセリング・リスニング | 末尾 |
| 10 | A8.net メディア会員募集(登録300円) | アフィリエイト・ブログ運営・ブログ記事 | 末尾 |
| 11 | リトルサーバー(レンタルサーバー、契約800〜2,500円) | レンタルサーバー・WordPress・ワードプレス・ブログを始め・ブログ開設 | 途中・末尾 |
| 12 | Quicca Plus(WordPress特化サーバー、申込額の30%) | レンタルサーバー・WordPress・ワードプレス・ブログ開設 | 末尾 |
| 13 | Qドメイン(初回ドメイン購入75円) | 独自ドメイン・ドメイン取得 | 末尾 |
| 14 | SiteLock(Webサイトの脆弱性診断・改ざん監視、100〜3,500円) | 改ざん・脆弱性・WordPressのセキュリティ・サイトのセキュリティ | 末尾 |
| 15 | ESET HOME セキュリティ(新規製品の購入15%) | セキュリティ・ウイルス・マルウェア・フィッシング・詐欺メール・情報漏えい・情報漏洩 | 末尾 |

- キーワードは記事の中心テーマ(タイトル・説明文・タグ)だけと照合するので、テーマを表す語にしている
- A8.net のサイト登録: サイト名「AI時短ラボ」、URL はブログの公開URL、カテゴリ「ソフトウェア」
- 参加中21件のうち載せないもの: 株式投資の銘柄情報(投資顧問契約の媒介にあたり素材・PR文の変更不可、テーマ外)、REN SIM、HUAWEI、
  ソファスタイル・カヴァース・ベッドスタイル(家具)、NeoBuds Pro 2(テキスト素材が長文のみ)、digxipop・dearmini(写真ギフト)、RingConn、
  楽天アフィリエイト(テキストコード未取得、楽天ツールでの作成推奨)、A8セルフバック用の2件(掲載素材なし・提携解除しないこと)、
  GMOクラウド iCLUSTA+(法人向けで読者層と合わない)、WEBROOT for Gamer(ゲーマー向け)、ノートン(確定率40%・EPC 0.46と低く、ESET を優先。リスティングは全ワード禁止)
- バナーは Notta 以外、ブラウザ操作ツールの制限で未取得。使う場合は A8.net の「広告リンク作成」→「バナー」→ 300×250 の素材をコピーする
- 各プログラムの主な注意: リスティング広告(社名・商品名での検索広告)は多くのプログラムで禁止。PLAUD は「議事録」「文字起こし」なども
  リスティングNGワード(このブログは検索広告を出していないので影響なし)。PLAUD・GMKtec は公式サイトの人物画像の使用NG。
  PLAUD は「提携から3か月以内に記事掲載がないと提携解除」の条件あり(議事録の記事に掲載済み)
...
| #17 | A8.net の参加中21件を確認し、広告を10件に拡充(アイディー・Aiarty・ExpressVPN・GMKtec・EarFun・A8.net 会員募集・ZENCHORD1 テキストを追加) |
...
- **バナー広告**: Notta 以外のバナー(300×250)は未取得。必要なら A8.net で素材をコピーして追加する
...
- **SNS 告知の実績確認**: 動画の告知は本番で成功済み(Issue #9)。ブログ記事の告知は 2026-09-26 朝が初回。改善後の動画紹介文(PR #10)は次の動画で初めて使われる
